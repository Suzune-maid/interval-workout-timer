...
>
> Last updated: **2026-05-11 22:58 CST**
>
> 本次 dated 記錄：`docs/plans/2026-05-11-w3d1-kegel-base-project-status-handoff.md`

## TL;DR

- **主要分支**：`main`
- **測試基線**：`npm test` → **87/87 pass**
- **語音庫覆蓋**：第一週 W1D1～W1D7（`2026-04-27`～`2026-05-03`）＋第二週 W2D1～W2D7（`2026-05-04`～`2026-05-10`）＋第三週 W3D1（`2026-05-11`）
...
- 以 live code 確認：**2026-05-11 = W3D1 = 凱格爾普通日**；預設載入會命中 `audio/library/2026-05-11/manifest.json`，不再 fallback。W3D1 已建立全新凱格爾 base（18 個 WAV），並通過 deterministic audio QA + Groq ASR transcript QA。
...
- 第三週 `audio/library/index.json` coverage 已開始補齊到 `2026-05-11`
...
### 4. 目前 live 日期已進入 W3D1，今天預設已有第三週凱格爾 base 語音
以 `timer-core.js` live 計算確認：

- `2026-05-11` → `W3D1`
- session title：`凱格爾普通日`
- `audio/library/2026-05-11/manifest.json` 已建立
- 預設載入今天時，會直接命中該 manifest，而不是 fallback

這點會影響 smoke test 與後續語音素材規劃：

- **預設載入（2026-05-11）現在應顯示 W3D1 專用語音資訊**
- fallback 驗證需改用 W3D2 以後尚未收錄 library 的日期，不要再把 W3D1 當成 fallback 範例
...
- 已跑 deterministic audio QA、7 個新 clip ASR fuzzy transcript QA、本機 browser smoke；目前 PASS，僅剩可接受 warning：`2026-05-10/guidance/phase-01-inhale.wav` leading silence
...
```bash
npm run audio:qa
npm run audio:qa:date -- 2026-05-11
npm run audio:verify:date -- 2026-05-11 --cache audio/library/2026-05-11/tts-reports/groq-asr-cache.json
git diff --check
```

結果：

- `npm run audio:qa` → PASS，所有日期 deterministic audio QA 無 error；`2026-05-11` 為 0 warnings
- `npm run audio:qa:date -- 2026-05-11` → PASS，checked=18 / errors=0 / warnings=0
- `npm run audio:verify:date -- 2026-05-11 --cache audio/library/2026-05-11/tts-reports/groq-asr-cache.json` → PASS，deterministic-audio 18/18，ASR transcript 18/18
- `git diff --check` → PASS
- Browser smoke（本機 `http://127.0.0.1:8124/`）已確認：
  - 預設載入 `2026-05-11`：UI 顯示 W3D1 / 凱格爾普通日，narration 命中 `audio/library/2026-05-11/manifest.json`
  - `2026-05-11` 播放鏈：`audio/library/2026-05-11/generated/phase-01.wav` → `audio/fx/countdown-start.wav` → `audio/library/2026-05-11/guidance/phase-01-inhale.wav`
...
1. **預設載入（2026-05-11）**
   - 今天是 W3D1，應直接載入 `audio/library/2026-05-11/manifest.json`
   - UI 應顯示 W3D1 凱格爾普通日專用語音資訊，而不是 fallback 文案
2. **切到 2026-05-08**
...
10. **按下開始**
   - 確認 `2026-05-11` 會先播 W3D1 phase narration，再進 start cue / 倒數 guidance
...
- 第二週已完整 coverage；第三週之後是否已有 coverage，仍要先查 `audio/library/index.json`

所以：

- 不要把「第二週都還沒有 library」當成前提
- 但也不要直接假設第三週以後日期已補齊
- 新增第三週或更後面的素材前，先查 `audio/library/index.json`
