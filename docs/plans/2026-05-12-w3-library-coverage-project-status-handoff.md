...
>
> Last updated: **2026-05-12 19:35 CST**
>
> 本次 dated 記錄：`docs/plans/2026-05-12-w3-library-coverage-project-status-handoff.md`

## TL;DR

- **主要分支**：`main`
- **測試基線**：`npm test` → **93/93 pass**
- **語音庫覆蓋**：第一週 W1D1～W1D7（`2026-04-27`～`2026-05-03`）＋第二週 W2D1～W2D7（`2026-05-04`～`2026-05-10`）＋第三週 W3D1～W3D7（`2026-05-11`～`2026-05-17`）
...
- 以 live code 確認：**2026-05-12 = W3D2 = 正式訓練日**；預設載入會命中 `audio/library/2026-05-12/manifest.json`，不再 fallback。第三週已完整補齊到 W3D7；凱格爾日 W3D4/W3D6 直接複用 W3D1 base。
...
- `2026-05-06`（W2D3）放鬆日 library manifest 已建立：3 段 phase narration + 9 句循環 countdown guidance，共 12 個 WAV
- `2026-05-07`～`2026-05-10`（W2D4～W2D7）library manifest 已建立：以既有同構素材複用為主，並新增 7 個 variation WAV 降低重複感
- `2026-05-11`（W3D1）凱格爾普通日 base 已建立：5 段 phase narration + 13 個 countdown guidance，共 18 個新 WAV；W3 節奏為 5 秒輕收 / 8 秒放、快速 12 次、反向 150 秒
- 第三週 `audio/library/index.json` coverage 已補齊到 `2026-05-17`
...
- `docs/plans/2026-05-07-w2-library-coverage-project-status-handoff.md` → 本次紀錄快照
- `docs/plans/2026-05-05-w2d3-project-status-handoff.md`
...
- `2026-05-06` — W2D3 — 放鬆日（新生 W2D3 降張力語音，3 段 narration + 9 句循環 guidance）
- `2026-05-07` — W2D4 — 凱格爾普通日（複用 W1D1，phase-01 開場旁白換成新 variation）
- `2026-05-08` — W2D5 — 正式訓練日（複用 W2D2，phase-01 @4s 與 phase-03 @135s 換成新 variation）
- `2026-05-09` — W2D6 — 凱格爾普通日（可依狀態改放鬆；複用 W1D1，phase-04 @4s 與 phase-05 @48s 換成新 variation）
- `2026-05-10` — W2D7 — 休息／輕放鬆日（複用 W1D7，inhale / exhale 循環 cue 換成新 variation）
- `2026-05-11` — W3D1 — 凱格爾普通日（全新 W3 kegel base；5 段 narration + 13 個 guidance，共 18 個 WAV；deterministic QA 與 Groq ASR 18/18 PASS）
- `2026-05-12` — W3D2 — 正式訓練日（全新 W3 formal base；5 段 narration + 18 個 guidance，共 23 個 WAV；deterministic QA 0 warnings，Groq ASR 23/23 PASS）
- `2026-05-13` — W3D3 — 放鬆日（複用 `2026-04-29`，並裁切來源 guidance leading silence；date QA 0 warnings）
- `2026-05-14` — W3D4 — 凱格爾普通日（直接複用 W3D1 base，`assetSourceDay = 2026-05-11`）
- `2026-05-15` — W3D5 — 正式訓練日（直接複用 W3D2 formal base，`assetSourceDay = 2026-05-12`）
- `2026-05-16` — W3D6 — 凱格爾普通日（可依狀態改放鬆；直接複用 W3D1 base，保留 adaptive title）
- `2026-05-17` — W3D7 — 休息／輕放鬆日（複用 `2026-05-10` 新版休息日 cue；date QA 0 warnings）

### 4. 目前 live 日期已進入 W3D2，今天預設已有第三週正式訓練語音
以 `timer-core.js` live 計算確認：

- `2026-05-12` → `W3D2`
- session title：`正式訓練日`
- `audio/library/2026-05-12/manifest.json` 已建立
- 預設載入今天時，會直接命中該 manifest，而不是 fallback

這點會影響 smoke test 與後續語音素材規劃：

- **預設載入（2026-05-12）現在應顯示 W3D2 正式訓練日專用語音資訊**
- fallback 驗證需改用第三週以後尚未收錄 library 的日期，不要再把 W3D1～W3D7 當成 fallback 範例
...
- 3 個 phase narration WAV
- 9 個 countdown guidance WAV（腹式呼吸、反向凱格爾、收尾掃描）
- 合計 12 個 WAV
- `audio/library/index.json` 已新增 `2026-05-06`
- 已跑 deterministic audio QA + ASR fuzzy transcript QA；目前 PASS，僅剩可接受 warnings（部分前後靜音與 `phase-02` low-confidence ASR）

`2026-05-07`～`2026-05-10` 的第二週剩餘素材已補齊：

- W2D4 / W2D6：以 `2026-04-27` 凱格爾普通日為同構複用來源
- W2D5：以 `2026-05-05` W2D2 正式訓練日為同構複用來源
- W2D7：以 `2026-05-03` 休息／輕放鬆日為同構複用來源
- 額外新增 7 個 variation WAV：W2D4 開場旁白 1 個、W2D5 guidance 2 個、W2D6 guidance 2 個、W2D7 inhale/exhale guidance 2 個
- 已跑 deterministic audio QA、7 個新 clip ASR fuzzy transcript QA、本機 browser smoke；目前 PASS。`2026-05-10/guidance/phase-01-inhale.wav` 的 leading silence 已裁切修正，W2D7 / W3D7 date QA 皆為 0 warnings。

`2026-05-11` 的 W3D1 凱格爾 base 已補齊：

- 5 個 phase narration WAV
- 13 個 countdown guidance WAV（phase 1 呼吸 2 個、phase 2 慢速收放 2 個、phase 3 快速收放 2 個、phase 4 反向呼吸 2 個、phase 5 收尾掃描 5 個）
- 合計 18 個新 WAV，全部落在 `audio/library/2026-05-11/`
- W3 節奏：慢速 `5 秒輕收 / 8 秒完整放掉 × 10`、快速 `1 秒點收 / 1 秒放掉 × 12`、反向 `4 秒吸氣下沉 / 8 秒吐氣保持鬆 × 12`，最後 6 秒安靜放鬆
- 已跑 `npm run audio:qa:date -- 2026-05-11`：PASS，0 errors / 0 warnings
- 已跑 `npm run audio:verify:date -- 2026-05-11 --cache audio/library/2026-05-11/tts-reports/groq-asr-cache.json`：PASS，ASR transcript 18/18
- 已跑 `npm test`：93/93 pass

`2026-05-12`～`2026-05-17` 的第三週剩餘素材已補齊：

- W3D2：全新正式訓練 base，5 段 phase narration + 18 個 countdown guidance，共 23 個 WAV
- W3D3：複用 `2026-04-29` 放鬆日，並修正來源 `phase-02-guidance-02.wav` leading silence
- W3D4 / W3D6：依主人指示直接複用新建的 W3D1 凱格爾 base；W3D6 保留「可依狀態改放鬆」metadata
- W3D5：直接複用 W3D2 formal base
- W3D7：複用 `2026-05-10` 新版休息日 cue，並修正來源 `phase-01-inhale.wav` leading silence
- 已跑 `npm run audio:qa:date -- 2026-05-12`、`-- 2026-05-13`、`-- 2026-05-17`：PASS，0 errors / 0 warnings
- 已跑 `npm run audio:verify:date -- 2026-05-12`：PASS，ASR transcript 23/23（2 個 acceptable low-confidence warnings）
...
npm run audio:qa:date -- 2026-05-17
npm run audio:verify:date -- 2026-05-12
git diff --check
```

結果：

- `npm run audio:qa` → PASS，所有日期 deterministic audio QA 無 error；W3D1～W3D7 date QA 皆為 0 warnings
- `npm run audio:qa:date -- 2026-05-12` → PASS，checked=23 / errors=0 / warnings=0
- `npm run audio:qa:date -- 2026-05-17` → PASS，checked=3 / errors=0 / warnings=0
- `npm run audio:verify:date -- 2026-05-12` → PASS，deterministic-audio 23/23，ASR transcript 23/23（2 個 low-confidence warning，判定 acceptable）
- `git diff --check` → PASS
- Browser smoke（本機 `http://127.0.0.1:8124/`）已確認：
  - 預設載入 `2026-05-12`：UI 應顯示 W3D2 / 正式訓練日，narration 命中 `audio/library/2026-05-12/manifest.json`
  - `2026-05-12` 播放鏈：`audio/library/2026-05-12/generated/phase-01.wav` → `audio/fx/countdown-start.wav` → `audio/library/2026-05-12/guidance/phase-01-guidance-01.wav`
  - 先前已驗過 `2026-05-07`、`2026-05-08`、`2026-05-10` 的 W2D4～W2D7 播放鏈
...
1. **預設載入（2026-05-12）**
   - 今天是 W3D2，應直接載入 `audio/library/2026-05-12/manifest.json`
   - UI 應顯示 W3D2 正式訓練日專用語音資訊，而不是 fallback 文案
2. **切到 2026-05-08**
   - W2D5 正式訓練日，應直接載入 `audio/library/2026-05-08/manifest.json`
   - phase-01 倒數第 4 秒應播 `audio/library/2026-05-08/guidance/phase-01-guidance-01.wav`
3. **切到 2026-05-10**
   - W2D7 休息／輕放鬆日，應直接載入 `audio/library/2026-05-10/manifest.json`
   - 呼吸循環應使用 `audio/library/2026-05-10/guidance/phase-01-inhale.wav` / `phase-01-exhale.wav`
4. **切到 2026-05-06**
   - W2D3 放鬆日，應直接載入 `audio/library/2026-05-06/manifest.json`
   - 播放鏈應為 phase narration → `audio/fx/countdown-start.wav` → countdown guidance（例如 `phase-01-guidance-01.wav` / `phase-01-guidance-02.wav`）
5. **切換日程表週次**
...
8. **切到 2026-05-03**
   - 休息日，單 phase 呼吸放鬆
9. **切到尚未收錄 library 的日期**
   - 確認會退回開始音效 + 文字腳本 fallback 模式
10. **按下開始**
   - 確認 `2026-05-12` 會先播 W3D2 phase narration，再進 start cue / 倒數 guidance
   - phase-03 快速收放 guidance 的 `audioDurationSeconds` 應短於下一個 same-track `replace-track` event 的間隔，避免 cue 被切斷
11. **手機螢幕喚醒**
...
### 8. 第三週之後的日期要重新看 library 覆蓋範圍

目前已補到：

- `2026-05-04`（W2D1）已有 library manifest
- `2026-05-05`（W2D2）已有完整正式訓練日 library manifest
- 第一～三週已完整 coverage；第四週之後是否已有 coverage，仍要先查 `audio/library/index.json`

所以：

- 不要把「第二週都還沒有 library」當成前提
- 但也不要直接假設第四週以後日期已補齊
- 新增第四週或更後面的素材前，先查 `audio/library/index.json`
...
- `docs/plans/2026-05-07-w2-library-coverage-project-status-handoff.md`（本次快照）
- `docs/plans/2026-05-05-w2d3-project-status-handoff.md`（上一份 W2D3 快照）
- `docs/plans/2026-05-05-project-status-handoff.md`（上一份同日快照）
...
- `audio/library/index.json`
- `audio/library/2026-05-10/manifest.json`（W2D7 休息日複用 + inhale/exhale variation 參考）
- `audio/library/2026-05-09/manifest.json`（W2D6 adaptive 凱格爾複用 + guidance variation 參考）
- `audio/library/2026-05-08/manifest.json`（W2D5 正式訓練日複用 + guidance variation 參考）
- `audio/library/2026-05-07/manifest.json`（W2D4 凱格爾複用 + 開場 variation 參考）
- `audio/library/2026-05-06/manifest.json`（W2D3 放鬆日完整降張力語音參考）
...
- `docs/plans/2026-05-07-w2-library-coverage-project-status-handoff.md`
- `docs/plans/2026-05-05-w2d3-project-status-handoff.md`
- `docs/plans/2026-05-05-project-status-handoff.md`
- `docs/plans/2026-05-04-project-status-handoff.md`
