...
>
> Last updated: **2026-05-07 20:09 CST**
>
> 本次 dated 記錄：`docs/plans/2026-05-07-w2-library-coverage-project-status-handoff.md`

## TL;DR

- **主要分支**：`main`
- **測試基線**：`npm test` → **86/86 pass**
- **語音庫覆蓋**：第一週 W1D1～W1D7（`2026-04-27`～`2026-05-03`）＋第二週 W2D1～W2D7（`2026-05-04`～`2026-05-10`）
...
- 以 live code 確認：**2026-05-07 = W2D4 = 凱格爾普通日**；預設載入會命中 `audio/library/2026-05-07/manifest.json`，不再 fallback。第二週 W2D4～W2D7 已用 reuse + 7 個新 variation clips 補齊。
...
- 第二週 `audio/library/index.json` coverage 已完整補齊到 `2026-05-10`
...
### 4. 目前 live 日期已進入 W2D4，今天預設已有第二週凱格爾日專用語音
以 `timer-core.js` live 計算確認：

- `2026-05-07` → `W2D4`
- session title：`凱格爾普通日`
- `audio/library/2026-05-07/manifest.json` 已建立
- 預設載入今天時，會直接命中該 manifest，而不是 fallback

這點會影響 smoke test 與後續語音素材規劃：

- **預設載入（2026-05-07）現在應顯示 W2D4 專用語音資訊**
- fallback 驗證需改用第三週以後尚未收錄 library 的日期，不要再把第二週任何一天當成 fallback 範例
...
```bash
npm run audio:qa
node scripts/verify-voice-library.mjs --manifest /tmp/interval-workout-timer-w2-variation-asr-manifest.json --cache /tmp/interval-workout-timer-w2-variation-asr-cache.json --request-delay-ms 3200 --groq-retries 1 --groq-retry-delay-ms 65000
git diff --check
```

結果：

- `npm run audio:qa` → PASS，所有日期 deterministic audio QA 無 error
- 7 個新 variation WAV 的 ASR fuzzy transcript QA → PASS
- `git diff --check` → PASS
- Browser smoke（本機 `http://127.0.0.1:8124/`）已確認：
  - `2026-05-07` 播放鏈：`audio/library/2026-05-07/generated/phase-01.wav` → `audio/fx/countdown-start.wav` → 既有 inhale guidance
  - `2026-05-08` 播放鏈：W2D2 phase narration → `audio/fx/countdown-start.wav` → `audio/library/2026-05-08/guidance/phase-01-guidance-01.wav`
  - `2026-05-10` 播放鏈：W1D7 phase narration → `audio/fx/countdown-start.wav` → `audio/library/2026-05-10/guidance/phase-01-inhale.wav` → `phase-01-exhale.wav`
...
1. **預設載入（2026-05-07）**
   - 今天是 W2D4，應直接載入 `audio/library/2026-05-07/manifest.json`
   - UI 應顯示 W2D4 凱格爾普通日專用語音資訊，而不是 fallback 文案
2. **切到 2026-05-08**
...
10. **按下開始**
   - 確認 `2026-05-07` 會先播 W2D4 phase narration，再進 start cue / 倒數 guidance
11. **手機螢幕喚醒**
