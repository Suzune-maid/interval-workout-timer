...
test('createNarrationPlayer 在 full 模式會用同一個 narration track 串接階段旁白與開始音效', async () => {
  const calls = [];
  ...
  await player.playPhaseIntro(0, 'full');

  assert.deepEqual(calls.map((call) => ({ track: call.track, src: call.src })), [
    { track: 'narration', src: './audio/library/test-fixtures/generated/phase-01.wav' },
    { track: 'narration', src: START_CUE },
  ]);
});
