...
test('buildDailySession 會依週次與星期建立正式訓練內容', () => {
  ...
  const advanced凱格爾Day = buildDailySession(6, 1);
  ...
  assert.equal(advanced凱格爾Day.kind, 'kegel');
  assert.ok(advanced凱格爾Day.phases.some((phase) => phase.label.includes('波峰模擬')));
});
...
test('buildNarrationEntries 會為課表建立可重用語音腳本與對應起始時間', () => {
  const session = {
    ...
    phases: [
      ...
      { label: '收尾放鬆', seconds: 60, cue: '可正常結束，最後做呼吸與反向凱格爾。' },
    ],
  };
