...
function installFakeWakeLock() {
  ...
  return { requests, releases, sentinels };
}
...
async function bootApp({
  ...
  return { calendar, document, timer, audio, wakeLock, restoreDate, fetchCalls: fetchState.fetchCalls };
}
...
test('按開始時會取得 screen wake lock，暫停時會釋放，恢復時會重新取得', async () => {
  const { document, timer, audio, wakeLock, restoreDate } = await bootApp();
  try {
    const startButton = document.querySelector('#start-button');
    ...
    pauseButton.click();
    await nextTurn();
    assert.deepEqual(wakeLock.releases, ['screen']);
...
test('按重設時會取消訓練並釋放 screen wake lock', async () => {
  const { document, audio, wakeLock, restoreDate } = await bootApp();
  try {
    const startButton = document.querySelector('#start-button');
